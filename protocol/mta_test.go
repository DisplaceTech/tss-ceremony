...
func TestMultiplicativeToAdditive_ZeroInput(t *testing.T) {
	n := secp256k1.S256().N

	// Test with k_a = 0 (should fail)
	_, _, err := MultiplicativeToAdditive(big.NewInt(0), big.NewInt(1))
	if err == nil {
		t.Error("expected error for k_a = 0, got nil")
	}

	// Test with k_b = 0 (should fail)
	_, _, err = MultiplicativeToAdditive(big.NewInt(1), big.NewInt(0))
	if err == nil {
		t.Error("expected error for k_b = 0, got nil")
	}

	// Test with k_a >= n (should fail)
	_, _, err = MultiplicativeToAdditive(n, big.NewInt(1))
	if err == nil {
		t.Error("expected error for k_a >= n, got nil")
	}

	// Test with k_b >= n (should fail)
	_, _, err = MultiplicativeToAdditive(big.NewInt(1), n)
	if err == nil {
